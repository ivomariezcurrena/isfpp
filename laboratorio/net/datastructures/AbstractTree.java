...
import java.util.Iterator;         // for use as snapshot iterator
...
public Iterable<Position<E>> listGreaterAncestor() {
  List<Position<E>> RamaLarga = new ArrayList<>();
    if (isEmpty()) return RamaLarga;
        ...
        for (Position<E> leaf : leaves) {
            List<Position<E>> currentPath = new ArrayList<>();
            Position<E> current = leaf;
            while (current != null) {
                currentPath.add(0, current);  // Add to the beginning to reverse the order
                current = parent(current);
            }

            if (currentPath.size() > RamaLarga.size()) {
                RamaLarga = currentPath;
            }
        }
