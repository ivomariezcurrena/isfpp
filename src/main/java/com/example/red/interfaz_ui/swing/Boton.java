...
public class Boton extends JButton {
    ...
    public Boton() {
        ...
        TimingTarget objetivo = new TimingTargetAdapter() {
            ...
            public void timingEvent(float fraccion) {
                ...
                tamanoAnimado = fraccion * tamanoObjetivo; // Ajusta el tamaño del efecto segun la fraccion de la
                                                           // animacion
    ...
    protected void paintComponent(Graphics grphcs) {
        ...
        BufferedImage imagen = new BufferedImage(ancho, alto, BufferedImage.TYPE_INT_ARGB); // Crea una imagen en
                                                                                            // memoria
        Graphics2D g2 = imagen.createGraphics(); // Obtiene el objeto para dibujar en la imagen
        g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON); // Activa el
                                                                                                 // anti-aliasing para
                                                                                                 // suavizar bordes
        g2.setColor(getBackground()); // Establece el color de fondo
        g2.fillRoundRect(0, 0, ancho, alto, alto, alto); // Dibuja un rectangulo redondeado con las dimensiones del
                                                         // boton
        if (puntoPresionado != null) {
            g2.setColor(colorEfecto); // Establece el color del efecto visual
            g2.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_ATOP, transparencia)); // Aplica la
                                                                                                 // transparencia
            g2.fillOval((int) (puntoPresionado.x - tamanoAnimado / 2), (int) (puntoPresionado.y - tamanoAnimado / 2),
                    (int) tamanoAnimado, (int) tamanoAnimado); // Dibuja el efecto de onda circular en el punto de
                                                               // presion
        }
